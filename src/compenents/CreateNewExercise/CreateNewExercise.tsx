...
export const CreateNewExercise = () => {
    ...
    const selectExercise =
        <select placeholder="Wprowadź nazwę ćwiczenia" defaultValue={exerciseList[0]?.name}
        ...
        <>
            ...
            <div className='create-exer'>
                <form className='create-exer-form' onSubmit={addExercise}>
                    ...
                    />
                    {newExercise.name === nameCheck ?
                        <p>W danym zestawie ćwiczeń już istnieje ćwiczenie o nazwie {nameCheck}</p> : null}
                        ...
                        {chosenExercises.length > 0 ? (
                            chosenExercises.map((exercise, index) => (
                                <li className='exer-li' key={exercise.name}>
                                    <p>Nazwa ćwiczenia: {exercise.name}
                                        <button className='del-exer-button'
                                                onClick={() => deleteExercise(exercise.name)}>Usuń
                                        </button>
                                    </p>
